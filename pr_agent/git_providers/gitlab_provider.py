...
class GitLabProvider(GitProvider):
    ...
    def __init__(self, merge_request_url: Optional[str] = None, incremental: Optional[bool] = False):
        ...
        self.id_mr = None
        self.mr = None
        self.id_changeset_project = None
        self.id_changeset_mr = None
        self.changeset_mr = None
    ...
    def get_changeset_content_from_mr(self, merge_request_url: str ):
        self.id_changeset_project, self.id_changeset_mr = self._parse_merge_request_url(merge_request_url)
        self.changeset_mr = self._get_changeset_merge_request()
        try:
            raw_changes = self.changeset_mr.changes().get('changes', [])
            raw_changes = self._expand_submodule_changes(raw_changes)
            diff = raw_changes[-1].get('diff')
            return diff
        except Exception as e:
            get_logger().error(f"Could not get diff for merge request {e}")
            return None

    def get_changeset_content_from_file(self, file_url: str ):
        try:
            ...
            project_path = "/".join(parts[:dash_idx])  # group[/subgroup]/project
            ref = parts[dash_idx + 2]  # commit SHA / branch / tag
            file_path = "/".join(parts[dash_idx + 3:])  # path/to/file

            if not project_path or not ref or not file_path:
                get_logger().error(f"Cannot parse blob URL components: {file_url}")
                return None
            file_obj = self.gl.projects.get(project_path).files.get(file_path, ref)
            content = file_obj.decode()
            return decode_if_bytes(content)
        except Exception as e:
            get_logger().error(f"GitLab get file failed for {file_url}: {e}")
            return None
    ...
    def _get_changeset_merge_request(self):
        mr = self.gl.projects.get(self.id_changeset_project).mergerequests.get(self.id_changeset_mr)
        return mr
    ...
    def add_feedback_reactions(self, comment):
        try:
            comment.awardemojis.create({
                'name': 'thumbsup'
            })
            comment.awardemojis.create({
                'name': 'thumbsdown'
            ...
            get_logger().warning(f"Failed to add feedback reactions, error: {e}")
