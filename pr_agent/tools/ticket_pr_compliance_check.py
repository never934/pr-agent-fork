...
jira_email = get_settings().get('jira.email')
jira_token = get_settings().get('jira.token')
jira = JIRA(
    server = "https://shartech.atlassian.net/",
...
async def extract_tickets(git_provider):
    ...
    try:
        if isinstance(git_provider, GithubProvider):
            ...
            user_description = git_provider.get_user_description()
            tickets = find_jira_tickets(user_description)
            tickets_content = []
            ...
            if tickets:
                try:
                    for ticket in tickets:
                        ...
                        issue_body_str = issue.fields.description or ""
                        if len(issue_body_str) > MAX_TICKET_CHARACTERS:
                            get_logger().warning(f"Limit of jira issue exceeded: {issue.key}")
                            issue_body_str = issue_body_str[:MAX_TICKET_CHARACTERS] + "..."

                        parent = getattr(issue.fields, 'parent', None)
                        if parent:
                            parentIssue = jira.issue(parent.key)
                            parent_issue_body_str = parentIssue.fields.description or ""
                            if len(parent_issue_body_str) > MAX_TICKET_CHARACTERS:
                                get_logger().warning(f"Limit of jira issue exceeded: {parentIssue.key}. Length: {len(parent_issue_body_str)}")
                                parent_issue_body_str = parent_issue_body_str[:MAX_TICKET_CHARACTERS] + "..."

                            tickets_content.append({
                                'ticket_id': parentIssue.key,
                                'ticket_url': parentIssue.key,
                                'title': parentIssue.fields.summary,
                                'body': parent_issue_body_str,
                                'labels': [],
                                'sub_issues': []  # Store sub-issues content
                            })
                    ...
                    get_logger().error(f"Error extracting jira ticket: {e}",)
