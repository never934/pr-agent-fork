...
from pr_agent.tools.pr_changeset import (
    extract_and_cache_changeset
)
...
class PRReviewer:
    ...
    def __init__(self, pr_url: str, is_answer: bool = False, is_auto: bool = False, args: list = None,
        ...
        self.vars = {
            ...
            "changeset": get_settings().get('changeset', None),
    ...
    async def run(self) -> None:
        try:
            ...
            await extract_and_cache_changeset(self.git_provider, self.vars)
            ...
            if get_settings().config.publish_output and not get_settings().config.get('is_auto_command', False):
                self.git_provider.publish_comment("Думаю...", is_temporary=True)
            ...
            if get_settings().pr_reviewer.persistent_comment and not self.incremental.is_incremental:
                ...
                comment = self.git_provider.publish_persistent_comment(pr_review,
                                                            initial_header=f"{PRReviewHeader.REGULAR.value} 🔍",
                                                            update_header=True,
                                                            final_update_message=final_update_message, )
            else:
                comment = self.git_provider.publish_comment(pr_review)

            self.git_provider.remove_initial_comment()
            self.git_provider.add_feedback_reactions(comment)
    ...
    def _prepare_pr_review(self) -> str:
        ...
        data = load_yaml(self.prediction.strip(),
                         keys_fix_yaml=["changeset_check", "ticket_compliance_check", "key_issues_to_review:",
