...
class PRReviewHeader(str, Enum):
    REGULAR = "## AI Ревью"
    INCREMENTAL = "## Incremental AI Ревью"
...
def convert_to_markdown_v2(output_data: dict,
    ...
    if get_settings().get("pr_reviewer.enable_intro_text", False):
        markdown_text += f"Несколько ключевых моментов, которые помогут в процессе проверки:\n\n"
    ...
    for key, value in output_data['review'].items():
        ...
        elif 'changeset check' in key_nice.lower():
            markdown_text = changeset_markdown_logic(emoji, markdown_text, value, gfm_supported, files, git_provider)
    ...
    markdown_text += "<strong>Оставьте отзыв:</strong> реагируйте на комментарий 👍, если предложение было полезным, или 👎 если полезным не было."
...
def changeset_markdown_logic(emoji, markdown_text, value, gfm_supported, files, git_provider) -> str:
    get_logger().info(f"Changeset markdown value: {emoji} {value}")
    if is_value_no(value):
        markdown_text += f"<tr><td>"
        markdown_text += f"{emoji}&nbsp;<strong>МР соотвествует ченджсету ✅</strong>"
        markdown_text += f"</td></tr>\n"
    else:
        if isinstance(value, list):
            ...
            for mismatch in value:
                try:
                    relevant_file = mismatch.get('relevant_file', '').strip()
                    mismatch_header = mismatch.get('mismatch_header', '').strip()
                    mismatch_content = mismatch.get('mismatch_content', '').strip()
                    start_line = int(str(mismatch.get('start_line', 0)).strip())
                    end_line = int(str(mismatch.get('end_line', 0)).strip())
                    ...
                    if gfm_supported:
                        if reference_link is not None and len(reference_link) > 0:
                            if relevant_lines_str:
                                mismatch_str = f"<details><summary><a href='{reference_link}'><strong>{mismatch_header}</strong></a>\n\n{mismatch_content}\n</summary>\n\n{relevant_lines_str}\n\n</details>"
                            else:
                                mismatch_str = f"<a href='{reference_link}'><strong>{mismatch_header}</strong></a><br>{mismatch_content}"
                        else:
                            mismatch_str = f"<strong>{mismatch_header}</strong><br>{mismatch_content}"
                    else:
                        if reference_link is not None and len(reference_link) > 0:
                            mismatch_str = f"[**{mismatch_header}**]({reference_link})\n\n{mismatch_content}\n\n"
                        else:
                            mismatch_str = f"**{mismatch_header}**\n\n{mismatch_content}\n\n"
                    mismatches += f"\n\n{mismatch_str}"
                except Exception as e:
                    get_logger().exception(f"Failed to process 'Changeset check': {e}")

            markdown_text += f"<tr><td>"
            markdown_text += f"{emoji}&nbsp;<strong>Найдены расхождения с Changeset'ом ❌</strong>\n<br><br>\n{mismatches}"
            markdown_text += f"</td></tr>\n"
        else:
            markdown_text += f"<tr><td>"
            markdown_text += f"{emoji}&nbsp;<strong>Changeset не найден 🧐</strong>"
            markdown_text += f"</td></tr>\n"
    return markdown_text
